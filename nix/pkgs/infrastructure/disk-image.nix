...
pkgs.stdenv.mkDerivation {
  ...
  buildPhase = ''
            ...
            # For root and user accounts (user has empty password for easy login)
            # Password hashes use argon2 format: $argon2id$...
            # Empty password means any password works (or just press enter)
    ...
    GROUP
            ...
            ${lib.optionalString (userutils != null) ''
              echo "Copying userutils..."
              if [ -d "${userutils}/bin" ]; then
                cp -rv ${userutils}/bin/* redoxfs-root/bin/ 2>/dev/null || true
                cp -rv ${userutils}/bin/* redoxfs-root/usr/bin/ 2>/dev/null || true
                echo "Copied userutils (getty, login, passwd, su, sudo)"
              else
                echo "WARNING: userutils not found at ${userutils}/bin"
              fi
            ''}
