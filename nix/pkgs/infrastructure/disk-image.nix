...

{
  ...
  sodium ? null,
  netutils ? null,
  # User management utilities (getty, login, passwd, su, sudo)
  userutils ? null,
...
pkgs.stdenv.mkDerivation {
  ...
  buildPhase = ''
                ...
                # (No password field - passwords stored separately in /etc/shadow)
                cat > redoxfs-root/etc/passwd << 'PASSWD'
    root;0;0;root;/root;/bin/ion
    user;1000;1000;Redox User;/home/user;/bin/ion
    PASSWD

                # Create group file
                # Format: groupname;password;gid;members
                cat > redoxfs-root/etc/group << 'GROUP'
    root;x;0;root
    user;x;1000;user
    GROUP

                # Create shadow file for password hashes
                # Format: username;passwordhash
                # Empty hash = blank password (press Enter to log in)
                # "!" = password not set (cannot log in with password)
                cat > redoxfs-root/etc/shadow << 'SHADOW'
    root;
    user;
    SHADOW
                ...
                # Copy userutils (getty, login, passwd, su, sudo, id, useradd, etc.)
                ${lib.optionalString (userutils != null) ''
                  echo "=== Copying userutils from ${userutils} ==="
                  echo "Contents of userutils/bin:"
                  ls -la ${userutils}/bin/ || echo "Failed to list ${userutils}/bin"
                  if [ -d "${userutils}/bin" ]; then
                    echo "Copying to redoxfs-root/bin/..."
                    cp -v ${userutils}/bin/* redoxfs-root/bin/
                    echo "Copying to redoxfs-root/usr/bin/..."
                    cp -v ${userutils}/bin/* redoxfs-root/usr/bin/
                    echo "=== Verifying login in redoxfs-root ==="
                    ls -la redoxfs-root/bin/login redoxfs-root/usr/bin/login || echo "login NOT FOUND!"
                  else
                    echo "ERROR: userutils not found at ${userutils}/bin"
                    exit 1
                  fi
                ''}
