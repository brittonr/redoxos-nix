  ...
  impl =
    ...
    let
      ...
      profileContent = ''
        ...
        ''}
        ${lib.optionalString hasSelfHosting ''
      ...
      allInitScripts =
        ...
        // (lib.optionalAttrs userutilsInstalled {
      ...
      // (lib.optionalAttrs (graphicsEnabled && pkgs ? orbterm) {
        "etc/orbterm/config" = {
          text = ''
      ...
      mkStorePackages = lib.concatStringsSep "\n" (
        builtins.map (pkg: ''
          ...
          ${lib.optionalString (isSelfHostingPkg pkg) ''
        ...
        ${lib.concatStringsSep "\n" (
          builtins.map (pkg: ''
            ...
            ${lib.optionalString (isSelfHostingPkg pkg) ''
              ...
              if [ -d "$out/nix/store/${pkgStoreName pkg}/lib" ]; then
                mkdir -p $out/nix/system/profile/lib
                for f in $out/nix/store/${pkgStoreName pkg}/lib/*; do
                  [ -e "$f" ] || [ -L "$f" ] || continue
                  lib_name=$(basename "$f")
                  ln -sf "/nix/store/${pkgStoreName pkg}/lib/$lib_name" \
        ...
        hostPkgs.runCommand "redox-root-tree"
          ...
          ''
            ...
            ${lib.optionalString hasSelfHosting ''
                        ...
                        ln -sf "/nix/store/${pkgStoreName sysrootPkg}/sysroot" \
                        ...
                        mkdir -p $out/lib
                        cp "/nix/store/${pkgStoreName sysrootPkg}/lib/ld64.so.1" $out/lib/ld64.so.1
                        chmod 555 $out/lib/ld64.so.1
                        cp "/nix/store/${pkgStoreName sysrootPkg}/lib/libc.so" $out/lib/libc.so
                        ...
                        if [ -d "$out/nix/store/${pkgStoreName rustcPkg}/lib" ]; then
                          for f in $out/nix/store/${pkgStoreName rustcPkg}/lib/*.so; do
                            [ -e "$f" ] || continue
                            lib_name=$(basename "$f")
                            ln -sf "/nix/store/${pkgStoreName rustcPkg}/lib/$lib_name" \
                              "$out/usr/lib/rustc/$lib_name"
                          done
                        fi

                        # Fix p_align=0 in ELF program headers (relibc ld_so bug workaround).
                        # relibc's dynamic linker does `p_vaddr % p_align` without checking for
                        # zero, causing a division-by-zero panic on segments like GNU_STACK.
                        # Patch all dynamically-linked ELF files to set p_align=1 where it's 0.
                        echo "Fixing p_align=0 in ELF program headers..."
                        python3 << 'FIXELF'
              import struct, os

              def fix_elf_palign(path):
              ...
              root = os.environ["out"]
              count = 0
              # Fix in nix/store/ (rustc .so files)
              for search_dir in [os.path.join(root, "nix", "store"), os.path.join(root, "lib")]:
                  if not os.path.isdir(search_dir):
                      continue
                  for dirpath, dirs, files in os.walk(search_dir):
                      for name in files:
                          if name.endswith('.so') or name.endswith('.so.6') or name in ('rustc', 'rustdoc'):
                              path = os.path.join(dirpath, name)
                              if os.path.islink(path):
                                  continue
                              st = os.stat(path)
                              os.chmod(path, st.st_mode | 0o200)
                              if fix_elf_palign(path):
                                  count += 1
                              os.chmod(path, st.st_mode)
              if count:
                  print(f"  Fixed p_align=0 in {count} ELF files")
              FIXELF
            ''}
