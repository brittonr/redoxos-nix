...

{
  ...
  impl =
    ...
    let
      ...
      # ===== INIT.RC =====
      initRc = ''
        ${if initfsEnableGraphics then "# Redox init with graphics" else "# Headless Redox init"}
        ...
        notify ptyd
        ${lib.optionalString initfsEnableGraphics ''
          notify inputd
          notify vesad
      ...
      initfs = hostPkgs.stdenv.mkDerivation {
        ...
        buildPhase = ''
          runHook preBuild
          mkdir -p initfs/{bin,lib/drivers,etc/pcid,usr/bin,usr/lib/drivers}
